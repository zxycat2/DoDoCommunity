...
exports.main = async (event, context) => {
  ...
  // if(event.avatarUrl == undefined || event.nickName == undefined){
  ...
  // 如果没有获取到，则新增数据
  if(user == undefined && event.creatingNewUser == true){
  ...
  }else if(user == undefined && event.creatingNewUser == false){
    var result = {}
    result.errCode = 2
    result.errMsg = '用户未进行过注册，将跳转至注册页'
  ...
  if(add_result){
    ...
    result.errCode = 0
    result.errMsg = '该用户已微信注册过，从云数据库中获取信息'
  }
